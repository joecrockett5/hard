...
class BaseObject(BaseModel):
    ...
    def to_db(self):
        ...
        partition_key = f"{as_dict.pop("user_id")}{DELIMITER}{as_dict.pop("object_type")}"
    ...
    def from_db(cls, object: dict[str, str]):
        try:
            ...
            raise ValueError(f"Invalid Partition: Object partition ({DB_PARTITION}) did not contain delimiter: {DELIMITER}")
        except KeyError:
            raise KeyError(f"Missing Partition: Object did not contain partition key ({DB_PARTITION})")
